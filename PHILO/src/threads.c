...
void *check_death(void *args)
{
	...
	while(*(life->dinning->someone_died))
	{
		...
		if(since_last_meal(life) >= life->dinning->options[time_to_die] && *(life->dinning->someone_died))
		{	
...
void *routine(void *args)
{
	...
	while(*(life->dinning->someone_died))
	{
		...
		{	
			...
			usleep(life->dinning->options[time_to_eat]);
		...
		if(*(life->dinning->someone_died))
		{
			...
			usleep(life->dinning->options[time_to_sleep]);
		}
		if(*(life->dinning->someone_died))
